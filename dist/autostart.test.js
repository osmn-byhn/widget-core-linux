...
describe('AutostartManager', () => {
    ...
    it('should enable autostart on macOS', () => {
        ...
        const success = manager.enable('test-id', 'Test Widget', 'node runner.js');
        expect(success).toBe(true);
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('Library/LaunchAgents/com.widget.test-id.plist'), expect.stringContaining('node runner.js'));
    });
});
